...
Rec = [path '/Output/3D_imgs/ParticleCentroid_%03d.txt'];
...
z_start = (z0 - Lz/2)/resolution; % [voxel] locations 2200/resolution
Lz = 15; % searching radius along Z axis
...
for i = 1:Nf
    ...
    s = importdata(sprintf( Rec, i));
    for j = 1:length(s.data)
        Rec_position(i,j,:) = s.data(j, 1:3); 
    end
    Num_recparticle(i) =  length(s.data);
end
...
figure(i);
subplot(2,2,1)
histogram(dx, 15); xlim([-1 1]); ylim([0 50]); xlabel('dx (vx)'); ylabel('Counts'); title('Meas. PDF - X'); % ylim([0 50]);
subplot(2,2,2)
histogram(dy, 15); xlim([-1 1]); ylim([0 50]); xlabel('dy (vx)'); ylabel('Counts'); title('Meas. PDF - Y'); % ylim([0 50]);
subplot(2,2,3)
histogram(dz, 15); xlim([-20 20]); ylim([0 80]); xlabel('dz (vx)'); ylabel('Counts'); title('Meas. PDF - Z'); % ylim([0 80]);
end
...
for i = 1:Nf
    ...
    plot3(GT_position(i,[index_gt_match{i} index_gt_ambiguity{i}],3),...
          ...
          GT_position(i,[index_gt_match{i} index_gt_ambiguity{i}],2),'ro','MArkerFaceColor','r', 'Markersize', 4);
    hold on;
...
%     hold on;
    ...
    plot3(Rec_position(i,index_rec_match{i},3),Rec_position(i,index_rec_match{i},1),Rec_position(i,index_rec_match{i},2),'bo','MArkerFaceColor','b', 'Markersize', 4);
    hold on;
    
    plot3(Rec_position(i,index_rec_ghost{i},3),Rec_position(i,index_rec_ghost{i},1),Rec_position(i,index_rec_ghost{i},2),'ko','MArkerFaceColor','k', 'Markersize', 4);
    hold on;
    
    plot3(Rec_position(i,index_rec_ambiguity{i},3),Rec_position(i,index_rec_ambiguity{i},1),Rec_position(i,index_rec_ambiguity{i},2),'go','MArkerFaceColor','green', 'Markersize', 4);
